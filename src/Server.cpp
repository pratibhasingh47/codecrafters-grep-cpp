...
std::vector<std::string> decompose_pattern(const std::string& pattern) {
    ...
    while (i < n) {
        size_t matched = 0;
        if (pattern[i] == '\\' && i+1 < n) {
            if (i+2 < n && quantifiers.contains(pattern[i+2])) {
                matched = 3;
            }
            else {
                matched = 2;
            ...
            if (k+1 < n && quantifiers.contains(pattern[k+1])) {
                matched = k - i + 2;
            }
            else {
                matched = k - i + 1;
            ...
            if (i+1 < n && quantifiers.contains(pattern[i+1])) {
                matched = 2;
            }
            else {
                matched = 1;
...
size_t match_pattern_at_beginning(const std::string& input_line, 
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (patterns[i].starts_with('(') && patterns[i].ends_with(')')) {
            ...
            while (pos < m && char_matches_pattern(input_line[pos], pattern)) {
                ...
                if (pattern.starts_with("[^") && match_pattern_at_beginning(input_line.substr(pos), to_check, nullptr, tmp)) {
                    break;
                }
                ++pos;
            }
